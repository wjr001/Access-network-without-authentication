...
int ReadSet(const char* SetName,char* value,int MaxLength)
{
    ifstream setfile;
    setfile.open("set.conf",ios::in);
    if (!setfile.is_open())
    {
        setfile.close();
    ...
    else
    {
        setfile >> nowset;
        setfile >> nowset;
        if (strlen(nowset)>MaxLength)
        {
            cout << "Item " << SetName << " out of length,max length: "<<MaxLength<<"\n";
            system("pause");
            exit(0);
        }
        else
...
int main()
{
    char* WaitTimeTemp= (char*)malloc(16);
    long int WaitTime=30000;
    ReadSet("WaitTime", WaitTimeTemp, 16);
    if ((AllisNum(WaitTimeTemp) == true))
    {
        stringstream chartoint;
        chartoint<<WaitTimeTemp;
        chartoint >> WaitTime;
    }
    else
    {
        cout << "WaitTime valu not pure numbers!";
        system("pause");
        exit(0);
    }
    cout << "请确认参数是否正确:"<<"\n发包间隔时间(ms): "<<WaitTime<<"\n";
